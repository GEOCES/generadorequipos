...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUsers = async () => {
    try {
      ...
      localStorage.setItem('team_generator_users', JSON.stringify(formattedUsers));
  ...
  const checkSession = async () => {
    try {
      // Verificar sesión en localStorage primero
      ...
      // Si no hay sesión local, verificar en Supabase
  ...
  const register = async (data: RegisterData): Promise<boolean> => {
    try {
      ...
      // Crear usuario directamente en nuestra tabla (sin Supabase Auth por ahora)
